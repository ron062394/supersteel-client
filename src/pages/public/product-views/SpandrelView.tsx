...
import type { TSpandrelProductData } from '../../../types/products';
...
const SpandrelView = () => {
  ...
  const [product, setProduct] = useState<TSpandrelProductData | null>(null);
  const [relatedProducts, setRelatedProducts] = useState<TSpandrelProductData[]>([]);
  ...
  const fetchProductData = (): void => {
      ...
      const foundProduct = getProductById(spandrel, id);
      if (foundProduct) {
        setProduct(foundProduct as TSpandrelProductData);
      } else {
  ...
  const fetchRelatedProducts = (): void => {
      const filteredProducts = spandrel.filter((p: TSpandrelProductData) => p.id !== id);
      const relatedProducts = getRelatedProducts(filteredProducts, id || '', 3);
      setRelatedProducts(relatedProducts as TSpandrelProductData[]);
  };
