...
import type { TLightFramesProductData } from '../../../types/products';

const LightFramesView = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const [currentImageIndex, setCurrentImageIndex] = useState<number>(0);
  const [product, setProduct] = useState<TLightFramesProductData | null>(null);
  const [relatedProducts, setRelatedProducts] = useState<TLightFramesProductData[]>([]);
  ...
  const fetchProductData = (): void => {
    try {
      ...
      const foundProduct = getProductById(lightFrames, id);
      if (foundProduct) {
        setProduct(foundProduct as TLightFramesProductData);
      } else {
  ...
  const fetchRelatedProducts = (): void => {
    try {
      ...
      const filteredProducts = lightFrames.filter((p: TLightFramesProductData) => p.id !== id);
      const relatedProducts = getRelatedProducts(filteredProducts, id || '', 3);
      setRelatedProducts(relatedProducts as TLightFramesProductData[]);
    } catch (error) {
